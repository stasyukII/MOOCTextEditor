...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		boolean newSlog = true;
		String glasnie = "aeiouy";
		for (int i = 0; i<chars.length; i++) {
			if (i == chars.length-1 && Character.toLowerCase(chars[i]) == 'e' 
		    		&& newSlog && syll > 0)
				syll--;
			if (newSlog && glasnie.indexOf(Character.toLowerCase(chars[i]))>=0) {
				syll++;
				newSlog = false;
			}
			else if (glasnie.indexOf(Character.toLowerCase(chars[i]))<0)
				newSlog = true;
	...
	public double getFleschScore()
	{
		...
		double score = 0;
		double first = (double)getNumWords()/(double)getNumSentences();
		double second = (double)getNumSyllables()/(double)getNumWords();
